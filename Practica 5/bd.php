<?php
    ...
    // obtiene los nombres de las actividades AJAX
    function getActividadesNombresSimilitud($title){
        global $mysqli;
    
        $stmt = $mysqli->prepare("SELECT id,titulo,publicado FROM Actividades WHERE  titulo LIKE '%$title%'");
    ...
    function insertarActividad($titulo, $precio, $fecha, $duracion, $descripcion, $ruta, $descripcionImg) {
        ...
        $stmt = $mysqli->prepare("INSERT INTO Actividades (titulo, precio, fecha, duracion, descripcion, publicado) VALUES (?, ?, ?, ?, ?, ?)");
    
        $publicado = 0;

        $stmt->bind_param("sssssi", $titulo, $precio, $fecha, $duracion, $descripcion, $publicado);
    ...
    function insertarImagen($ruta_destino,$descripcionImagen, $idactividad) {
        ...
        // editar comentario
        function editarVisibilidad($id, $publicado){
            global $mysqli;
            ...
            $stmt = $mysqli->prepare("UPDATE Actividades SET publicado = ? WHERE id = ?");
            $modificado = 1;
            $stmt->bind_param("ii", $modificador, $id);
            
            if ($stmt->execute()) {
                $stmt->close();
                return;
            } else {
                echo "Error al actualizar la visibilidad: " . $stmt->error;
                $stmt->close();
                return;
            }
        }
