...
function procesaRespuestaAjax(respuesta) {
    ...
    document.getElementById("BoxActividades").style.display = "none";
    console.log(Imagenes);
    if (respuesta.results && respuesta.results.length > 0) {
        for (let i = 0; i < respuesta.results.length; i++) {
            let actividad = respuesta.results[i];
            if(actividad.publicado == 1 || rol == 'gestor' || rol == 'super' ){
 
                if (actividad.publicado == 0) {
                    res += `<a class="BoxActividadesBox" style="background-color: rgba(255, 0, 0, 0.4);"  href="actividad.php?id=${actividad.id}">`;
                } else {
                    res += `<a class="BoxActividadesBox" href="actividad.php?id=${actividad.id}">`;
                }
    
                for (let j = 0; j < Imagenes.length; j++) {
                    let imagen = Imagenes[j];
                    if (imagen.principal == 1 && imagen.idActividad == actividad.id) {
                        res += `<img class="ImagenActividadPortada" src="${imagen.urlImagen}">`;
                    }
                }
            ...
            console.log(respuesta.results[i].id);
            console.log(respuesta.results[i].titulo);
        }
    } else {
        res = '<p style=" margin-left: 10px">No se encontraron resultados.</p>';
    }
...
function editarvisibilidad(publicado){

    if(publicado){
        Swal.fire({
            title: "¿Actualmente esta actividad esta publicada está seguro de cambiar a no publicada?",
            icon: "question",
            showCancelButton: true,
            confirmButtonText: "Si",
            cancelButtonText: "Cancelar",
            ...
            if (result.isConfirmed) {
                Swal.fire({
                    icon: "success",
                    text: 'Visibilidad cambiada',
                }).then((result) => {
                    if (result.isConfirmed) {
                        document.getElementById("Formulariocambiarvisibilidad").submit();
                    }
                });
            }
        });
        ...
        Swal.fire({
            title: "¿Actualmente esta actividad no esta publicada está seguro de cambiar a publicada?",
            icon: "question",
            showCancelButton: true,
            confirmButtonText: "Si",
            cancelButtonText: "Cancelar",
